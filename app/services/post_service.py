...
def update_post(
    db: Session, current_user: CurrentUser, id: int, post_data: post.PostUpdate
):
...
def fetch_posts_of_user(
    db: Session, current_user: CurrentUser, id: int, skip: int, limit: int
):
    if current_user.id != id and "admin" not in current_user.roles:
        raise HTTPException(
            status_code=403, detail="Unauthorized: You don't have access"
        )
